...
description: >
  ...
  image generator. This skill takes TWO inputs: the original CONCEPT + SCENES
  output from ad-idea-generator AND the per-scene prompts from
  video-prompt-builder. Use this skill as the third stage of a four-stage
  short-form video pipeline (ad-idea-generator → video-prompt-builder →
  reference-sheet-prompt-generator → starting-frame-prompt-generator → image
  gen → Seedance video gen). The reference images produced from this skill's
  output must exist before the starting-frame-prompt-generator runs, because
  starting frame prompts identify entities by the SUBJECT IDs and ENV IDs
  this skill assigns. The skill is silent-video aware. Do not use this skill
  for ad ideation, video shot-design, single-image art prompts, or anything
...
3. **You** (`reference-sheet-prompt-generator`) produce reference sheet prompts: one composite character sheet per character, one one-line product reference, one composite environment sheet per environment.
4. **`starting-frame-prompt-generator`** runs *after* this skill (and after the reference images have been generated by your pipeline's image generator) to produce per-scene starting frame prompts that explicitly reference the existing reference images by SUBJECT ID and ENV ID.
...
You receive both, together, as a single combined input from the upstream pipeline:

**Input A — The original CONCEPT + SCENES** from `ad-idea-generator`. This is the story-level truth: who is in the video, what happens, what the twist is, the tone, the texture. Use this to identify every distinct character, the product, and every distinct environment, and to gather the story-level specifics that inform their physical descriptions.

**Input B — The per-scene cinematic documents** from `video-prompt-builder`, one per scene. Each contains a SHOT-BY-SHOT EFFECTS TIMELINE and supporting analytical sections. Use this to confirm which entities actually appear in the video and to pick up any visual specifics mentioned in the shot descriptions (e.g. clothing wrinkles, lighting tones, posture details).

Reference sheets draw primarily from Input A. Input B is supplemental — it confirms entities and adds visual specifics where the shot prose mentioned them.
...
**Step 1 — Read Input A (CONCEPT + SCENES).**
...
**Step 3 — Cross-reference Input B (per-scene prompt-builder documents).**
Confirm every entity actually appears in the video. Pick up any visual specifics mentioned in the shot descriptions that should be reflected in the reference sheets (specific clothing details, lighting tones for environments, posture cues).

**Step 4 — Produce output in the order: characters, product, environments.**
...
**Input B (abbreviated — Shot 1 of each scene):**

> Scene 1, Shot 1 — Cool teal-and-amber Fincher thriller grade, hard side-light, deep shadows. Wide static shot of gift shop interior. Man in late 20s frozen mid-aisle, sweat at hairline.
> Scene 2, Shot 1 — Slow forward dolly-in. Subtle warm bloom edges in from right, cool grade still dominant.
> Scene 3, Shot 1 — Static wide, warm domestic grade. Young woman on couch holding paper card, interior angled away, face holds the realisation.
