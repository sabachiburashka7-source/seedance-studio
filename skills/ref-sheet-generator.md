...
MASTER SPATIAL MAP: Back Wall — [what surface / feature defines the far wall: specific material, any fixed object pinned to it]. Left Wall — [what defines the left boundary: architecture, window, shelving]. Right Wall — [what defines the right boundary: architecture, door, furniture]. Center Floor — [flooring material, any central furniture or open space]. Anchor Objects: [Object A] pinned to [Wall/position, distance from corner or fixed edge]; [Object B] pinned to [position]; these positions are immutable across all three panel views.

PROXIMITY CONSTRAINTS: [Object A] and [Object B] are not on the same wall — maintain a clear [X]-meter open floor gap between them in every panel. [Object C] does not touch [Object D]. State the void explicitly; the empty floor space is part of the layout.

GLOBAL ANCHOR: [Window / ceiling fixture / architectural feature] is the Global Anchor — it must be visible in every panel view, or its directional light-cast must read consistently across all panels to provide unified spatial orientation.

Panel 1 — View from [Camera Origin Point, e.g., "Center Floor rear position"] looking toward [Target, e.g., "Back Wall"]: [wide view — architecture, dominant furniture, ambient light]. Panel 2 — View from [Camera Origin Point, e.g., "Right Wall position"] looking toward [Target, e.g., "Left Wall"]: [medium view — mid-room relationships, how the Global Anchor lights the space]. Panel 3 — View from [Camera Origin Point, e.g., "close to the Global Anchor"] looking toward [Target, e.g., "Center Floor"]: [close-up detail — surface texture, objects, lived-in state].

[Architectural specifics: walls, flooring, ceiling]. [Window / lighting source description with quality of light — consistent with Global Anchor]. [Furniture and key objects in the space — specific, named, positions matching Spatial Map]. [Clutter / state / lived-in quality]. [Overall mood adjective phrase]. [Color tone description]. Color swatches bottom right. Annotation lines pointing to key elements. Photorealistic interior photography. Label top left: ENV ID: 0XX. Label top right: [SHORT LOCATION NAME] — [STATE OR MOOD DESCRIPTOR IN ALL CAPS]. Hyper-realistic, no illustration, no cartoon, no 3D render.
```

Required elements every environment sheet must contain:
- **MASTER SPATIAL MAP** block naming Back Wall, Left Wall, Right Wall, Center Floor, and Anchor Objects with immutable pinned positions
- **PROXIMITY CONSTRAINTS** block explicitly stating the open floor gap between key objects
- **GLOBAL ANCHOR** declaration — one window, ceiling fixture, or architectural feature that reads consistently across all panels
- **Camera Origin Point for every panel** — each panel described as "View from [origin] looking toward [target]," never just "a shot of [object]"
- Multi-panel layout (typically three panels: wide / medium / close-up), each with a camera origin
...
- Specific architectural detail (walls, floor, windows, lighting source) consistent with the Spatial Map
- Specific furniture and objects with positions matching the declared Anchor Object pins
...
MASTER SPATIAL MAP: Back Wall — cream-painted plaster, wooden counter with brass cash register and low stool pinned to the rear-right corner. Left Wall — floor-to-ceiling honey-toned wooden shelves pinned the full length of the left wall. Right Wall — floor-to-ceiling wooden shelves plus the front window with gauze curtains pinned to the right-front corner. Center Floor — pale grey wood-plank flooring, open walking aisle running front-to-back with no furniture blocking. Anchor Objects: counter pinned rear-right corner; window pinned front-right; wooden stepladder pinned left wall, two metres from rear — positions are immutable across all panels.

PROXIMITY CONSTRAINTS: The wooden counter (rear-right) and the stepladder (left wall, mid-room) are on opposite walls — maintain a clear 2.5-metre open aisle between them in every panel. No shelving unit crosses the center floor aisle. The plush-toy cluster sits only on upper-left shelves, not on the counter.

GLOBAL ANCHOR: The front window with gauze curtains (right-front corner) is the Global Anchor — its diffuse daylight wash must read consistently in every panel view, providing the primary directional light that falls left-to-right across the space.

Panel 1 — View from Center Floor rear position looking toward Back Wall: wide view of the full shop — counter rear-right, shelves flanking both walls, center aisle open, Global Anchor window light falling from the right, overhead pendant bulbs supplementing from above. Panel 2 — View from Right Wall position (near window) looking toward Left Wall: medium view of the left shelving run — paper-wrapped candles, ceramic mugs, card displays, stepladder mid-left, Global Anchor window out of frame right but its light falling across the left shelves. Panel 3 — View from Left Wall position (beside stepladder) looking toward Center Floor: close-up of a densely packed shelf section — woven baskets, dried-flower vases, small botanical prints, a trailing potted plant at floor level, warm pendant light above.

Cream-painted plaster walls with slight wear, honey-toned wooden shelves floor to ceiling on three walls, pale grey wood-plank flooring slightly scuffed. Front-right window with thin gauze curtains, diffuse soft daylight, supplemented by warm overhead pendant bulbs on simple cords. Counter at rear right: small brass cash register, low stool, a short glass display case. Shelves: paper-wrapped pillar candles, pastel ceramic mugs, framed botanical prints, woven baskets, dried-flower glass vases, paper card standing displays, plush toys on upper-left shelves. Wooden stepladder left wall. Trailing potted plant in left rear corner. Handwritten paper ceiling sign, text blurred, angled away from camera. Overall mood: cosy, slightly overstuffed, the lived-in feel of a shop unchanged for thirty years. Warm honey and amber tone with cream highlights. Color swatches bottom right. Annotation lines pointing to honey-toned shelves, gauze window, brass cash register, stepladder, plush-toy upper shelf. Photorealistic interior photography. Label top left: ENV ID: 001. Label top right: GIFT SHOP — DAYTIME RETAIL STATE. Hyper-realistic, no illustration, no cartoon, no 3D render.
...
MASTER SPATIAL MAP: Back Wall — soft warm-white painted concrete, sage-green velvet two-seater sofa pinned flush against the back wall center, framed botanical print on the wall above it. Left Wall — single window with thin curtains mostly drawn, pinned left-wall center. Right Wall — wooden shelving unit with trailing plants and books pinned against the right wall. Center Floor — pale honey wood-plank flooring, cream area rug centered under the sofa, low wooden coffee table pinned on the rug 60 cm in front of the sofa. Anchor Objects: sofa pinned back-wall center; window pinned left-wall center; floor lamp pinned left of sofa, 30 cm gap between lamp base and sofa arm — positions immutable across all panels.

PROXIMITY CONSTRAINTS: The floor lamp (left of sofa) and the shelving unit (right wall) are on opposite sides of the room — maintain a clear 2-metre open floor gap between them in every panel. The coffee table does not touch the sofa; 60 cm of rug shows between them. No furniture crowds the left wall below the window.

GLOBAL ANCHOR: The tall floor lamp beside the sofa (left of center) is the Global Anchor — its warm amber uplight must read consistently in every panel as the dominant warm-light source, counterbalanced by the cool blue window bleed from the left wall.

Panel 1 — View from Center Floor front position looking toward Back Wall: wide view of the full living room — sofa back-center, botanical print above it, floor lamp left of sofa casting warm amber up-light, window left-wall with cool blue evening bleed, coffee table in mid-foreground on cream rug, right shelving unit just visible right edge. Panel 2 — View from Right Wall position (beside shelving unit) looking toward Left Wall: medium view — sofa left-center with throw blanket draped over right arm, floor lamp prominent, Global Anchor warm light falling across sofa, window at far left casting cool-blue edge light, cream rug and coffee table (stack of books, ceramic mug, white candle) in mid-ground. Panel 3 — View from close to the Global Anchor (beside floor lamp) looking toward Center Floor: close-up of coffee table surface — three stacked books, small ceramic mug, single white candle, the warm amber lamp light falling across the table top, cream rug texture below.

Soft warm-white painted concrete walls with very slight unevenness, pale honey wood-plank flooring, cream area rug under the sofa. Left-wall window thin curtains mostly drawn, cool blue evening diffuse light. Floor lamp warm amber bulb, main practical light source. Kitchen pendant just out of frame right casting a secondary amber glow. Sage-green velvet two-seater sofa against back wall, cream throw draped over one arm. Framed botanical print above sofa. Low wooden coffee table: three stacked books, ceramic mug, white candle. Wooden shelving unit right wall: trailing plants, blurred framed photographs, small books. Kitchen counter edge in foreground right, wooden cutting board on it. Overall mood: intimate, lived-in, soft evening warmth. Warm amber and honey tones balanced against cool blue window bleed. Color swatches bottom right. Annotation lines pointing to sage-green sofa, floor lamp, left window cool bleed, coffee table candle, cream area rug. Photorealistic interior photography. Label top left: ENV ID: 002. Label top right: APARTMENT — EVENING DOMESTIC STATE. Hyper-realistic, no illustration, no cartoon, no 3D render.
```
